...
// ===================================================
// TAKE CANVAS — PURE WORK AREA (R4-004a)
// ===================================================
...
interface ColumnNode {
    ...
    height: number
    zIndex: number
    data: ColumnData & { expandedHeight?: number }
}
...
export const TakeCanvas = forwardRef<TakeCanvasHandle, TakeCanvasProps>(
    function TakeCanvas({ takeId, initialNodes, initialEdges, onNodesChange, initialUndoHistory, onUndoHistoryChange }, ref) {
        ...
        const pushHistory = useCallback(() => {
            ...
            if (h.stack.length > HISTORY_MAX) { h.stack.shift() } else { h.cursor++ }
            emitHistoryChange()
        }, [emitHistoryChange])
        ...
        const createNodeAt = useCallback((x: number, y: number) => {
            const newNode: NoteNode = {
                ...
                width: 200, height: 120, zIndex: nodesRef.current.length + 1, data: {},
            }
        ...
        const createImageNodeAt = useCallback((x: number, y: number, imageData: ImageData) => {
            ...
            if (naturalWidth > MAX_INITIAL_IMAGE_WIDTH || naturalHeight > MAX_INITIAL_IMAGE_HEIGHT) {
                if (ratio > MAX_INITIAL_IMAGE_WIDTH / MAX_INITIAL_IMAGE_HEIGHT) { w = MAX_INITIAL_IMAGE_WIDTH; h = w / ratio }
                else { h = MAX_INITIAL_IMAGE_HEIGHT; w = h * ratio }
            } else { w = naturalWidth; h = naturalHeight }
            const newNode: ImageNode = {
                id: crypto.randomUUID(), type: 'image',
                x: Math.round(x - w / 2), y: Math.round(y - h / 2),
                width: Math.round(w), height: Math.round(h),
                zIndex: nodesRef.current.length + 1, data: imageData,
            }
            setNodes(prev => [...prev, newNode])
            setSelectedNodeIds(new Set([newNode.id]))
            setSelectedEdgeId(null); setEditingEdgeLabel(null)
        ...
        // ── R4-004a: Create column ──
        const createColumnNodeAt = useCallback((x: number, y: number) => {
            const newNode: ColumnNode = {
                id: crypto.randomUUID(), type: 'column',
                x: Math.round(x - COLUMN_DEFAULT_WIDTH / 2), y: Math.round(y - COLUMN_DEFAULT_HEIGHT / 2),
                width: COLUMN_DEFAULT_WIDTH, height: COLUMN_DEFAULT_HEIGHT,
        ...
        useEffect(() => {
            ...
            selBoxRef.current = null; connectionRef.current = null
        ...
        const handleSelBoxMouseMove = useCallback((e: MouseEvent) => {
            ...
            setSelectionBoxRect({
                left: Math.min(sb.startX, currentX), top: Math.min(sb.startY, currentY),
                width: Math.abs(currentX - sb.startX), height: Math.abs(currentY - sb.startY),
            })
        }, [])
        ...
        const handleConnectionMouseMove = useCallback((e: MouseEvent) => {
            ...
            setConnectionGhost({ fromX: from.x, fromY: from.y, toX, toY })
        }, [])
        ...
        const handleConnectionMouseUp = useCallback((e: MouseEvent) => {
            ...
            const conn = connectionRef.current
            if (conn) {
                const mouseX = e.clientX - conn.canvasRect.left
                const mouseY = e.clientY - conn.canvasRect.top
                ...
                if (targetNode) {
                    const duplicate = edgesRef.current.some(
                        e => (e.from === conn.fromNodeId && e.to === targetNode.id) || (e.from === targetNode.id && e.to === conn.fromNodeId)
                    )
                    if (!duplicate) {
                        const newEdge: CanvasEdge = { id: crypto.randomUUID(), from: conn.fromNodeId, to: targetNode.id }
        ...
        const handleConnectionStart = useCallback((nodeId: string, mouseX: number, mouseY: number) => {
            ...
            connectionRef.current = { fromNodeId: nodeId, startX: mouseX - canvasRect.left, startY: mouseY - canvasRect.top, canvasRect }
        ...
        const handleResizeMouseMove = useCallback((e: MouseEvent) => {
            ...
            if (resizeRef.current.aspectRatio !== null) {
                ...
                if (newHeight < MIN_IMAGE_SIZE) { newHeight = MIN_IMAGE_SIZE; newWidth = newHeight * resizeRef.current.aspectRatio }
            } else {
            ...
            setNodes(prev => prev.map(node =>
                node.id === resizeRef.current!.nodeId ? { ...node, width: Math.round(newWidth), height: Math.round(newHeight) } : node
            ))
        }, [])
        ...
        const handleResizeStart = useCallback((nodeId: string, mouseX: number, mouseY: number) => {
            ...
            if (!node) return
            // R4-004a: columns not resizable when collapsed
            if (node.type === 'column' && (node.data as ColumnData).collapsed) return
            const aspectRatio = node.type === 'image' ? node.width / node.height : null
            resizeRef.current = { nodeId, startWidth: node.width, startHeight: node.height, startMouseX: mouseX, startMouseY: mouseY, aspectRatio }
        ...
        const handleRequestHeight = useCallback((nodeId: string, requestedHeight: number) => {
            setNodes(prev => prev.map(node =>
                node.id === nodeId && requestedHeight > node.height ? { ...node, height: requestedHeight } : node
            ))
            emitNodesChange()
        }, [emitNodesChange])

        // ============================================
        // R4-004a: COLUMN COLLAPSE TOGGLE
        // ============================================

        const handleToggleCollapse = useCallback((nodeId: string) => {
            setNodes(prev => prev.map(node => {
                if (node.id !== nodeId || node.type !== 'column') return node
                const col = node as ColumnNode
                const isCollapsed = col.data.collapsed ?? false

                if (!isCollapsed) {
                    // Collapsing: save current height, set to header-only
                    return {
                        ...col,
                        data: { ...col.data, collapsed: true, expandedHeight: col.height },
                        height: COLUMN_COLLAPSED_HEIGHT,
                    }
                } else {
                    // Expanding: restore saved height
                    const restoreHeight = col.data.expandedHeight ?? COLUMN_DEFAULT_HEIGHT
                    return {
                        ...col,
                        data: { ...col.data, collapsed: false, expandedHeight: undefined },
        ...
        const handlePotentialDragStart = useCallback((nodeId: string, mouseX: number, mouseY: number) => {
            const currentSelected = selectedNodeIdsRef.current.has(nodeId) ? selectedNodeIdsRef.current : new Set([nodeId])
            const offsets = new Map<string, { startX: number; startY: number }>()
            nodesRef.current.forEach(node => {
                if (currentSelected.has(node.id)) offsets.set(node.id, { startX: node.x, startY: node.y })
            })
            dragRef.current = { nodeId, offsets, startMouseX: mouseX, startMouseY: mouseY, hasMoved: false }
        ...
        const handleDelete = useCallback((nodeId: string) => {
            ...
            setSelectedNodeIds(new Set()); setSelectedEdgeId(null); setEditingEdgeLabel(null)
            setInteractionMode('idle'); setEditingField(null)
            setTimeout(() => { pushHistory(); emitNodesChange() }, 0)
        }, [pushHistory, emitNodesChange])
        ...
        const handleStartEditing = useCallback((nodeId: string, field: 'title' | 'body') => {
            setSelectedNodeIds(new Set([nodeId]))
            setSelectedEdgeId(null); setEditingEdgeLabel(null)
            setInteractionMode('editing'); setEditingField(field)
        }, [])

        const handleFieldFocus = useCallback((field: 'title' | 'body') => { setEditingField(field) }, [])

        const handleFieldBlur = useCallback(() => { setInteractionMode('idle'); setEditingField(null) }, [])

        const handleDataChange = useCallback((nodeId: string, data: NoteData | ColumnData) => {
            setNodes(prev => prev.map(node => node.id === nodeId ? { ...node, data: data as any } : node))
            setTimeout(() => { pushHistory(); emitNodesChange() }, 0)
        }, [pushHistory, emitNodesChange])
        ...
        const handleSvgClick = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
            ...
            edgesRef.current.forEach(edge => {
                ...
                const from = edgeAnchor(fromNode, nodeCenter(toNode).x, nodeCenter(toNode).y)
                const to = edgeAnchor(toNode, nodeCenter(fromNode).x, nodeCenter(fromNode).y)
                const dist = distToSegment(mouseX, mouseY, from.x, from.y, to.x, to.y)
                if (dist < closestDist) { closestDist = dist; closestEdge = edge }
            })
            if (closestEdge) {
                e.stopPropagation()
                setSelectedEdgeId(closestEdge.id); setEditingEdgeLabel(null)
                setSelectedNodeIds(new Set())
            }
        }, [])
        ...
        const handleCanvasMouseDown = useCallback((e: React.MouseEvent) => {
            ...
            setSelectedNodeIds(new Set()); setSelectedEdgeId(null); setEditingEdgeLabel(null); setEditingField(null)
        ...
        const edgeLines = edges.map(edge => {
            ...
            const fromC = nodeCenter(fromNode), toC = nodeCenter(toNode)
            return { edge, from: edgeAnchor(fromNode, toC.x, toC.y), to: edgeAnchor(toNode, fromC.x, fromC.y) }
        ...
        return (
            <div ref={canvasRef} className="flex-1 bg-zinc-950 relative overflow-hidden" onMouseDown={handleCanvasMouseDown}>
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
                    ...
                    <g style={{ pointerEvents: 'stroke' }} onClick={handleSvgClick as any}>
                        {edgeLines.map(({ edge, from, to }) => (
                            <line key={`hit-${edge.id}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y}
                                stroke="transparent" strokeWidth={EDGE_HIT_DISTANCE * 2} style={{ cursor: 'pointer', pointerEvents: 'stroke' }} />
                        ))}
                    </g>
                    {edgeLines.map(({ edge, from, to }) => {
                        const isSel = edge.id === selectedEdgeId
                        return <line key={edge.id} x1={from.x} y1={from.y} x2={to.x} y2={to.y}
                            stroke={isSel ? '#3b82f6' : '#71717a'} strokeWidth={isSel ? 2 : 1.5}
                            markerEnd={isSel ? 'url(#arrowhead-selected)' : 'url(#arrowhead)'} />
                    })}
                    {connectionGhost && (
                        <line x1={connectionGhost.fromX} y1={connectionGhost.fromY} x2={connectionGhost.toX} y2={connectionGhost.toY}
                            stroke="#10b981" strokeWidth={2} strokeDasharray="6 3" markerEnd="url(#arrowhead)" />
                    )}
                </svg>
                ...
                {edgeLines.map(({ edge, from, to }) => {
                    ...
                    return (
                        <div key={`label-${edge.id}`} className="absolute pointer-events-auto z-[5]"
                            style={{ left: midX, top: midY, transform: 'translate(-50%, -50%)' }}
                            onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}>
                            {isSelected ? (
                                <input type="text" autoFocus={editingEdgeLabel === edge.id}
                                    placeholder="label..." defaultValue={edge.label || ''}
                                    onFocus={() => setEditingEdgeLabel(edge.id)}
                                    onBlur={ev => {
                                        const value = ev.target.value.trim()
                                        setEdges(prev => prev.map(ed => ed.id === edge.id ? { ...ed, label: value || undefined } : ed))
                                        setEditingEdgeLabel(null)
                                        setTimeout(() => { pushHistory(); emitNodesChange() }, 0)
                                    }}
                                    onKeyDown={ev => {
                                        if (ev.key === 'Enter') (ev.target as HTMLInputElement).blur()
                                        if (ev.key === 'Escape') { (ev.target as HTMLInputElement).value = edge.label || ''; (ev.target as HTMLInputElement).blur() }
                                        ev.stopPropagation()
                                    }}
                ...
                {nodes.map(node => (
                    <NodeShell
                        ...
                        onDelete={handleDelete} onResizeStart={handleResizeStart}
                    ...
                    >
                        {node.type === 'note' ? (
                            <NoteContent
                                ...
                                onFieldFocus={handleFieldFocus} onFieldBlur={handleFieldBlur}
                ...
                {selectionBoxRect && selectionBoxRect.width > 2 && selectionBoxRect.height > 2 && (
                    <div data-selection-box className="absolute border border-blue-500 bg-blue-500/10 pointer-events-none z-[9998]"
                        style={{ left: selectionBoxRect.left, top: selectionBoxRect.top, width: selectionBoxRect.width, height: selectionBoxRect.height }} />
                )}
